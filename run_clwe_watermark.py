...
def main(args):
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        print("Reversed no-watermark score:", no_w_metric)
        print("Reversed image hCLWE score", w_metric)
        print("Reversed no_w latents avg diff", torch.mean(torch.abs(reversed_latents_no_w - init_latents_no_w)))
        print("Reversed w latents avg diff", torch.mean(torch.abs(reversed_latents_w - init_latents_w)))

        # reversed_latents_no_w_fft = torch.fft.fftshift(torch.fft.fft2(reversed_latents_no_w), dim=(-1, -2)).to(torch.cfloat)
        # reversed_latents_w_fft = torch.fft.fftshift(torch.fft.fft2(reversed_latents_w), dim=(-1, -2)).to(torch.cfloat)
        # no_w_metric = get_hclwe_score(reversed_latents_no_w_fft, secret, gamma)
        # w_metric = get_hclwe_score(reversed_latents_w_fft, secret, gamma)
        # print("Reversed no-watermark score:", no_w_metric)
        # print("Reversed image hCLWE score", w_metric)
        # print("Reversed no_w latents avg diff", torch.mean(torch.abs(reversed_latents_no_w - init_latents_no_w)))
        # print("Reversed w latents avg diff", torch.mean(torch.abs(reversed_latents_w - init_latents_w)))
